...
applies this function to each columns (hence the `2`) of the matrix `mu`.
...
This part is **sometimes a bit disillusioning** when seen for the first time:
...
on $R^2$ is to use the adjusted $R^2$, which can also be found in the R output (`Adjusted R-squared`).

#### Separating property of regression due to $R^2$: {#seperating_property}
...
# Draw two distributions for weights = x_low_high into one diagram
...
Every time we fit a model, we should check the assumptions [above](#_model_assumptions).
...
([Westfall](https://www.routledge.com/Understanding-Regression-Analysis-A-Conditional-Distribution-Approach/Westfall-Arias/p/book/9780367493516?srsltid=AfmBOore3O_Ciecl0TTkr9AjPIY1d6OmbQa7o7IAdKpTSkD8s9HkwzD4), p.21).
...
We create $100$ bootstrap replicates of our data set `d2` by drawing with 
...
*Observational* data was collected and we have no control over the weights.
