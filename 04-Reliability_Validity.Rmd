...
- In the biased case, there *is* as systematic difference between Mary and Peter.
  ...
  be even lower ($ICC1 = 0.61$, $ICC2 = 0.65$ -> verify exercise). 
  The unbiased columns would of course stay the same.
- ICC2 now consideres the bias of 5 degrees. The model knows about the shift.
...
\begin{table}[h]
  ...
  \begin{tabular}{lcc}
  \hline
  \textbf{Groups} & \textbf{Name} & \textbf{Variance} \\
  \hline
  \multicolumn{3}{l}{\textbf{Model 1}} \\
  ID & (Intercept) & 267.79 \\
  Residual &  & 53.75 \\
  \hline
  \multicolumn{3}{l}{\textbf{Model 2}} \\
  ID & (Intercept) & 270.882 \\
  Rater & (Intercept) & 6.193 \\
  Residual &  & 47.557 \\
  \hline
  \end{tabular}
  \caption{Variance Components for Models}
  \label{tab:variance_models}
\end{table}
