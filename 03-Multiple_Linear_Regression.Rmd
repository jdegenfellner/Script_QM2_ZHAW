...
The red line is a [loess smoothing](https://en.wikipedia.org/wiki/Local_regression) line
...
Now, let's fit the same model as [above](#adding_transformed_predictor_bayes) in the Bayesian framework.
...
- **Prediction** (i.e. guessing the outcome as good as possible) seems to be easier 
  ...
  [best subset selection](https://www.sthda.com/english/articles/37-model-selection-essentials-in-r/155-best-subsets-regression-essentials-in-r/). 
...
The output of the `anova` command shows us the residual degrees of freedom (`Res.Df`)
...
*carrying a lighter* and *lung cancer*, you would find an association. But if you condition on *smoking*, the association
...
mod1 <- lm(leg_right ~ leg_left, data = d)
mod2 <- lm(height ~ leg_left, data = d)
mod3 <- lm(height ~ leg_right, data = d)

# Function to create the regression equation text
reg_eq <- function(model, xvar = "x", yvar = "y") {
  coef <- coef(model)
  paste0(yvar, " = ",
         round(coef[1], 2), " + ",
         round(coef[2], 2), " × ", xvar)
...
# Plot 1: Leg Left vs Leg Right
p1 <- ggplot(d, aes(x = leg_left, y = leg_right)) +
  geom_point(color = "blue") +  # scatter points
  geom_smooth(method = "lm", se = FALSE, color = "black") +  # regression line
  annotate("text",
           x = min(d$leg_left, na.rm = TRUE),
           y = max(d$leg_right, na.rm = TRUE),
           label = reg_eq(mod1, "Leg Left", "Leg Right"),
           hjust = 0, vjust = 1, size = 3.5) +
  labs(x = "Leg Left", y = "Leg Right") +
  ggtitle("Leg Left vs Right") +
  theme(plot.title = element_text(hjust = 0.5))

# Plot 2: Leg Left vs Height
p2 <- ggplot(d, aes(x = leg_left, y = height)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  annotate("text",
           x = min(d$leg_left, na.rm = TRUE),
           y = max(d$height, na.rm = TRUE),
           label = reg_eq(mod2, "Leg Left", "Height"),
           hjust = 0, vjust = 1, size = 3.5) +
  labs(x = "Leg Left", y = "Height") +
  ggtitle("Leg Left vs Height") +
  theme(plot.title = element_text(hjust = 0.5))

# Plot 3: Leg Right vs Height
p3 <- ggplot(d, aes(x = leg_right, y = height)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  annotate("text",
           x = min(d$leg_right, na.rm = TRUE),
           y = max(d$height, na.rm = TRUE),
           label = reg_eq(mod3, "Leg Right", "Height"),
           hjust = 0, vjust = 1, size = 3.5) +
  labs(x = "Leg Right", y = "Height") +
  ggtitle("Leg Right vs Height") +
  theme(plot.title = element_text(hjust = 0.5))
...
# Fit the model with both leg lengths
m4.6 <- quap(
  alist(
    height ~ dnorm(mu, sigma),
...
**With the given regression model, one asks the question**: 
...
In the example below, the second predictor $X_2$ is a *perfect* linear 
...
- **Draw a DAG** (directed acyclic graph) of the hypothesized relationships between 
...
which can be criticized. Let's quickly visualize the distribution of *BPSysAve* and overlay
a normal density with parameters estimated from the data ($\mu$ sample mean of *BPSysAve*, 
...
My guess is: no. A first glance into the [literature](https://journals.sagepub.com/doi/10.1177/00368504221101636) 
...
check_model(mod)
qqPlot(mod) # bad
check_model(mod, check = "linearity") # only linearity
```
...
summary(mod)
check_model(mod) # show all plots
qqPlot(mod) 
```
...
Since $BMI$ is on a pipe from *PhysActive* to *BPSysAve*, it makes sense,
that the effect of *PhysActive* is smaller when we include $BMI$ in the model.
...
(Note: It is surprisingly hard to include a left-skewed distribution in `rethinking`.
Currently trying to resolve this issue.)

**Second**, we further try to model $\sigma$ as a function of $\mu$.
For higher expected values of *BPSysAve*, we would expect higher variability.
Hence, we try to **explicitly model heteroscedasticity**.
...
hist(rlnorm(10000, meanlog = 5, sdlog = 0.5))
# -> meanlog should be around 5!, not 140!
...
m_NHANES_lnorm <- quap(
  ...
  data = df,
  start = list(beta_0 = 4, beta_1 = c(0.5, 0), 
  beta_2 = 0, beta_3 = c(0.5, 0), beta_4 = 1, beta_5 = 0)
)

precis(m_NHANES_lnorm, depth = 2)

# Expected difference in blood pressure (mmHg):
set.seed(122)
...
# otherwise we get numerical explosions...
...
# Expected values on BPSysAve scale
E_active <- exp(clip(mu_active + 0.5 * sigma_active^2))
E_inactive <- exp(clip(mu_inactive + 0.5 * sigma_inactive^2))

# (distribution of) Expected difference in blood pressure (mmHg)
diff_expected_BP <- E_active - E_inactive

# Clean
diff_expected_BP_clean <- diff_expected_BP[is.finite(diff_expected_BP) & 
                                          abs(diff_expected_BP) < 20]
...
**Next**, let's visualize the posterior predictive distributions of 100 samples:
...
### [M] Exercise 19 {#exercise19_multiple_regression}
...
![](images/DAGS_6M3_McEleath.png){ width=80% }

### [M] Exercise 20 {#exercise20_multiple_regression}

Consider the DAG in McElreath page 187 where one wants to determine
the total causal effect of *Number of waffle  houses* (W) on 
*divorce rate* (D).
...
- Repeat the **entire content** we have covered in this lecture so far. 
  ...
  Specifically, **note all your open questions** about our content so far
...
- Go through the [NHANES example](https://jdegenfellner.github.io/Script_QM2_ZHAW/multiple-linear-regression.html#example_nhanes)
...
- Glimpse into our next chapter (if there is time left):
  [Reliability](https://jdegenfellner.github.io/Script_QM3_ZHAW/reliability-and-validity.html)
