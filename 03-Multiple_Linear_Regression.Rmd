...
The one adjustment set proposed by `dagitty` tells us that we should include
...
First, we need to test, if Age and BMI are independent, *if* we condition on Gender and Physical Activity.
This is the case.
...
We could try a **multiple linear regression model** here.
...
df_age %>%
  ...
  bins = 30, fill = "lightblue", alpha = 0.6) +  # Ensure density scaling
  geom_density(color = "blue", linewidth = 1) +  # Smoothed density of actual data
  stat_function(
    fun = dnorm, 
...
df <- df %>%
  mutate(
    PhysActive_index = as.integer(as.factor(PhysActive)),  # Convert to factor index (1-based)
    Gender_index = as.integer(as.factor(Gender))  # Convert to factor index (1-based)
  )
library(data.table)
df <- as.data.table(df)
...
```{r}
mod <- lm(BPSysAve ~ PhysActive + Age + Gender  + BMI, data = df)
summary(mod)
check_model(mod)
qqPlot(mod) # 
```
...
**Notice this**: The interpretation of a whole paper could change by falsely including
...
- Factors, categorical variables in model...
