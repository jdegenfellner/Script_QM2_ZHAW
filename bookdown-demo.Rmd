...
There is [formula](https://en.wikipedia.org/wiki/Cook%27s_distance#Relationship_to_other_influence_measures_(and_interpretation))
...
This is basically the same plot as above, just [detrended](https://cran.r-project.org/web/packages/qqplotr/vignettes/introduction.html). 
...
Go to the resuials [above](#normality_assumption) where we tested the normality assumption.
...
**Usually**, one would fit this model by throwing in all variables into the model, including BMI.
...
**There is also the $ICC_{agreement}$**, which explicitely considers this
...
- **Single_random_raters ICC2:** 
  ICC2 and ICC3 are based on the **same** statistical model. The only difference is
...
- **Single_fixed_raters ICC3:**
  ICC3 is based on the **same model as ICC2**, but assumes that the raters are fixed. 
