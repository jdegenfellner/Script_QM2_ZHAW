...
and set it to zero. Solve for $c$ and you have the minimum. Let's do that:
...
- `Residual standard error`: The standard deviation of the residuals $r_i = height_i - \hat{c}$.
    In this case identical with the standard error 
...
data.frame(height_i = 1:nrow(d2), height = d2$height) %>%
  ggplot(aes(x = height_i, y = height)) +
  geom_point() + 
  geom_hline(yintercept = mean(d2$height), color = "red") + 
...
### [M] Exercise 3 {#exercise2_Intro}
Verify analytically that the `Residual standard error` is identical with the standard error of the heights.
...
fig <- plot_ly() %>%
  ...
  add_surface(x = x, y = y, z = z_highlight, colorscale = "Viridis", opacity = 0.9) %>%
  layout(scene = list(
    xaxis = list(title = "X"),
    yaxis = list(title = "Y"),
    zaxis = list(title = "Density")
