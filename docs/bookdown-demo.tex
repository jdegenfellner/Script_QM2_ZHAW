...
electric current (I) at a resistance (R) in a circuit and find that \href{https://en.wikipedia.org/wiki/Ohm\%27s_law}{Ohm's law} \(V = IR\) is not true.
...
the law would be falsified. It is also true that the law is probably not 100\% accurate,
...
observing \(66\) or more findings would be very unlikely. If such an event would
...
They are not magical tools turning data into truth. Firstly, the rule garbage in, garbage out (GABA) applies.
...
(RMSE), \href{https://en.wikipedia.org/wiki/Receiver_operating_characteristic\#:~:text=The\%20area\%20under\%20the\%20curve,ranks\%20higher\%20than\%20'negative'}{Area Under the Curve}
...
\href{https://replicationindex.com/2020/12/30/a-meta-scientific-perspective-on-thinking-fast-and-slow/}{check} if the studies
...
In my opinion, we should never be afraid to test our statistical models (as honestly as possible) against reality.
...
\item
  ...
  of dollars in revenue offsetting the statisticians salary by a large margin.
\item
  ...
  than the average, will make you fabulously rich.
\item
...
So, \(y\) and \(x\) are not fixed but afflicted with uncertainty.
...
We want to model the adult height of the !Kung San people
...
or at least a mixture of normal distributions (female/male).
...
allows a rather large range of possible values for \(\mu\)
(the unobserved population mean of the !Kung San people).
\(\sigma\) (the unobserved standard deviation of the population of !Kung San people)
...
\textbf{Visualisation of the model structure}:
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-4-1.pdf}}
...
from \(21,873\) Swiss people is around \(25.67\) cm (\hyperref[exercise1_Intro]{Exercise 1}).

Remember, in the Bayesian world, there is no \textbf{fixed but unknown}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-5-1.pdf}}

A wide range of population means is possible. One could discuss this distribution
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-6-1.pdf}}
...
\textbf{Visualisation of the prior for heights}:
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-7-1.pdf}}
...
Now, there are a couple of different ways to estimate the model incorporating
...
\begin{verbatim}
##             mean        sd       5.5%      94.5%
## mu    154.604257 0.4119663 153.945856 155.262659
## sigma   7.730807 0.2913364   7.265195   8.196419
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-11-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-11-2.pdf}}
...
influence each other. This is also visible in the visualisation of the model structure:
...
\begin{verbatim}
##                 mu        sigma
## mu    0.1697162616 0.0001572608
## sigma 0.0001572608 0.0848769180
\end{verbatim}
...
\begin{verbatim}
##                mu       sigma
## mu    1.000000000 0.001310279
## sigma 0.001310279 1.000000000
\end{verbatim}

Let's plot the posterior in 3D, because we \textbf{can}:

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/posterior-3d-correct-1.pdf}}

\textbf{How beautiful is that?}
...
Here, we do not use any prior knowledge, at least not that explicitly.
...
true but unknown \(c\).
Every time we estimate a parameter, we put a hat over it.
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-16-1.pdf}}
...
\item
  ...
  for the Bayesian and the frequentist version considering
...
Go to the \hyperref[bivariate_normal]{section} about the bivariate normal distribution below.
...
If you look at it from above, we have highlighted the section on the surface
...
Calculate this probability with R:
...
Welche der folgenden Aussage(n) hinsichtlich \emph{Prediction} und \emph{Explanation} im
Kontext statistischer Modellierung ist/sind korrekt?

\begin{itemize}
\tightlist
\item
  \emph{Prediction models}, die gut funktionieren, lassen im Allgemeinen
  auch Rückschlüsse auf die zugrunde liegenden kausalen Zusammenhänge zu.
\item
  \emph{Prediction models} fokussieren sich auf die Güte der Prognose
  für neue Daten, nicht auf das Verständnis kausaler Beziehungen.
\item
  \emph{Explanatory models} benötigen eine gute theoretische
  Fundierung, um kausale Zusammenhänge zwischen Variablen testen zu können.
\item
  Ein Beispiel für ein \emph{Explanatory model} ist die Klassifikation von Bildern
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-21-1.pdf}}
...
\item
  Gäbe es noch eine dritte Variable - z.B. \emph{geographische Region} - die sowohl \(\mu\) als auch
...
means that we put a penalty on an error. The farther our prediction
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-22-1.pdf}}
...
This relationship is neither causal, nor deterministic.
...
\item
  ...
  If it was deterministic, we would not need statistical modeling.
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-23-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-25-1.pdf}}
...
\NormalTok{        a }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\FloatTok{171.1}\NormalTok{, }\DecValTok{20}\NormalTok{),}
...
\begin{verbatim}
##              mean         sd        5.5%       94.5%
## a     154.6001022 0.27030753 154.1680986 155.0321059
## b       0.9032812 0.04192363   0.8362791   0.9702832
## sigma   5.0718802 0.19115470   4.7663781   5.3773824
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-27-1.pdf}}

Note, that the credible intervals in the plot of the coefficients are
hardly noticeable. The reason is that the intercept \(a\) is rather large,
whereas \(b\) and sigma are comparatively small. The plot is scaled to the
largest parameter.
...
parameters in the multivariate (because three parameters simultaneously)
...
\begin{verbatim}
##          a          b      sigma 
## 0.07306616 0.00175759 0.03654012
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-29-2.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# both posterior plots seem symmetric}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-29-3.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-30-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-31-1.pdf}}
...
applies this function to each columns (hence the \texttt{2}) of the matrix \texttt{mu}.
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-32-1.pdf}}
...
This part is \textbf{sometimes a bit disillusioning} when seen for the first time:
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-35-1.pdf}}
...
since the errors are independent from each other) conditionally normally distributed
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-37-1.pdf}}
...
no matter how complicated the estimator is. We do not need formulae.
We simply
...
In the Bayesian setting, we used the centered weight variable. Let's do this here too for
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-42-1.pdf}}
...
\(\beta_i = \frac{y_i}{x_i}\).
Since we want to find the underlying process, we want to avoid this.

Although not perfect, one way to mitigate the influence of ``too many'' variables
on \(R^2\) is to use the adjusted \(R^2\), which can also be found in the R output (\texttt{Adjusted\ R-squared}).

\subsubsection{\texorpdfstring{Separating property of regression due to \(R^2\):}{Separating property of regression due to R\^{}2:}}\label{seperating_property}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-43-1.pdf}}

In the left plot, given \(X = x_{low}\) gives a rather strongly shifted normal distribution of
potentially observable heights for this weight compared to \(X=x_{high}\).
We would have a lower misclassification error when trying to distinguish heights
...
Every time we fit a model, we should check the assumptions \hyperref[_model_assumptions]{above}.
...
(\href{https://www.routledge.com/Understanding-Regression-Analysis-A-Conditional-Distribution-Approach/Westfall-Arias/p/book/9780367493516?srsltid=AfmBOore3O_Ciecl0TTkr9AjPIY1d6OmbQa7o7IAdKpTSkD8s9HkwzD4}{Westfall}, p.21).
...
Furthermore, the assumptions refer to the data generating process, not the data itself.
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-45-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-46-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-48-1.pdf}}
...
At least with a lag of \(1\), there is no large correlation between the residuals.
...
but the assumption of normality would be fulfilled.
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-53-1.pdf}}
...
and the residuals. Holding Cook's distance constant at 0.5,
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-54-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-56-1.pdf}}

The normality assumption seems to hold. Note that in the above QQ plot,
we detrended the residuals and used a bootstrap confidence band. Depending
...
We create \(100\) bootstrap replicates of our data set \texttt{d2} by drawing with
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-58-1.pdf}}
...
\emph{Observational} data was collected and we have no control over the weights.
...
\tightlist
\item
  Calculate mean and standard deviation from the residuals of the model
...
we only get a different amount of data points across the range of \(x\).
...
\tightlist
\item
  Review all content from the script, up to and including section 3.2.
\item
...
\item
  ...
  The article should not be too technical.
  If you send me one of these studies in advance, we will discuss the
...
Wir erinnern uns an die \emph{conditional distribution} Definition der Regression (Frequentistische Sicht):
...
Welche Voraussetzung(en) gilt/gelten im klassischen linearen Regressionsmodell?
...
there is a linear regression using a quadratic term for weight.
...
we want it to measure (curvature in our case). For instance, if we want to model a quadratic relationship (parabola) between
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-59-1.pdf}}

It would not be a good idea to fit a linear trend through this data,
because we would not capture the relationship adequately.
The red line is a \href{https://en.wikipedia.org/wiki/Local_regression}{loess smoothing} line
...
the overall linear trend is positive.
...
\begin{verbatim}
##             mean        sd       5.5%     94.5%
## a     146.672834 0.3736378 146.075688 147.26998
## b1     21.399170 0.2900474  20.935618  21.86272
## b2     -8.419959 0.2813480  -8.869607  -7.97031
## sigma   5.749788 0.1743170   5.471195   6.02838
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-60-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-61-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-61-2.pdf}}
...
mechanisms (for didactic reasons).

We use rather uninformative priors and fit the model using \texttt{quap}:
...
## b2     1.0095082 0.03899992 0.9471788  1.0718376
## sigma  1.8738834 0.13250804 1.6621099  2.0856568
\end{verbatim}
...
by drawing from the posterior distribution and compare these with the actually
...
\CommentTok{\# see Statistical Rethinking p 138.}
...
\CommentTok{\# summarize samples across cases}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-63-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-64-1.pdf}}
...
Note that we did not explicitly assume normally distributed errors in the model definition above,
...
Now, let's fit the same model as \hyperref[adding_transformed_predictor_bayes]{above} in the Bayesian framework.
...
\item
  ...
  in this case we conclude that the true relationship is captured rather well.
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-67-1.pdf}}

This is, of course, a very idealized situation. There is no curvature in the plane,
no interaction, no outliers, no heteroscedasticity. It's the simplest case of multiple regression
...
This is a very complex question. At this point we can say this:
...
\item
  ...
  all subsets of predictors. If that is not possible, one could use clever algorithms, like
  \href{https://www.sthda.com/english/articles/37-model-selection-essentials-in-r/155-best-subsets-regression-essentials-in-r/}{best subset selection}.
\item
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-69-1.pdf}}
...
The output of the \texttt{anova} command shows us the residual degrees of freedom (\texttt{Res.Df})
...
such metric, because RSS is monotonically decreasing with number of variables added and reaches
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-72-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-73-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-74-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-75-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-75-2.pdf}}
...
related to the very large sample size and a strict cutoff (\(\alpha = 0.05\)) makes no sense.
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-78-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-78-2.pdf}}
...
cigarettes and how often do non-smokers carry a lighter just for fun?
\emph{Smoking} is (causally) associated with \emph{lung cancer}. If you just look at the association between
\emph{carrying a lighter} and \emph{lung cancer}, you would find an association. But if you condition on \emph{smoking}, the association
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-79-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-79-2.pdf}}
...
\CommentTok{\# Fit simple linear regression models}
\NormalTok{mod1 }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(leg\_right }\SpecialCharTok{\textasciitilde{}}\NormalTok{ leg\_left, }\AttributeTok{data =}\NormalTok{ d)}
\NormalTok{mod2 }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(height }\SpecialCharTok{\textasciitilde{}}\NormalTok{ leg\_left, }\AttributeTok{data =}\NormalTok{ d)}
\NormalTok{mod3 }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(height }\SpecialCharTok{\textasciitilde{}}\NormalTok{ leg\_right, }\AttributeTok{data =}\NormalTok{ d)}

\CommentTok{\# Function to create the regression equation text}
\NormalTok{reg\_eq }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(model, }\AttributeTok{xvar =} \StringTok{"x"}\NormalTok{, }\AttributeTok{yvar =} \StringTok{"y"}\NormalTok{) \{}
\NormalTok{  coef }\OtherTok{\textless{}{-}} \FunctionTok{coef}\NormalTok{(model)}
  \FunctionTok{paste0}\NormalTok{(yvar, }\StringTok{" = "}\NormalTok{,}
         \FunctionTok{round}\NormalTok{(coef[}\DecValTok{1}\NormalTok{], }\DecValTok{2}\NormalTok{), }\StringTok{" + "}\NormalTok{,}
         \FunctionTok{round}\NormalTok{(coef[}\DecValTok{2}\NormalTok{], }\DecValTok{2}\NormalTok{), }\StringTok{" × "}\NormalTok{, xvar)}
\NormalTok{\}}

\CommentTok{\# Plot 1: Leg Left vs Leg Right}
\NormalTok{p1 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(d, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ leg\_left, }\AttributeTok{y =}\NormalTok{ leg\_right)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{color =} \StringTok{"blue"}\NormalTok{) }\SpecialCharTok{+}  \CommentTok{\# scatter points}
  \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{, }\AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{color =} \StringTok{"black"}\NormalTok{) }\SpecialCharTok{+}  \CommentTok{\# regression line}
  \FunctionTok{annotate}\NormalTok{(}\StringTok{"text"}\NormalTok{,}
           \AttributeTok{x =} \FunctionTok{min}\NormalTok{(d}\SpecialCharTok{$}\NormalTok{leg\_left, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
           \AttributeTok{y =} \FunctionTok{max}\NormalTok{(d}\SpecialCharTok{$}\NormalTok{leg\_right, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
           \AttributeTok{label =} \FunctionTok{reg\_eq}\NormalTok{(mod1, }\StringTok{"Leg Left"}\NormalTok{, }\StringTok{"Leg Right"}\NormalTok{),}
           \AttributeTok{hjust =} \DecValTok{0}\NormalTok{, }\AttributeTok{vjust =} \DecValTok{1}\NormalTok{, }\AttributeTok{size =} \FloatTok{3.5}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Leg Left"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Leg Right"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"Leg Left vs Right"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme}\NormalTok{(}\AttributeTok{plot.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{))}

\CommentTok{\# Plot 2: Leg Left vs Height}
\NormalTok{p2 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(d, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ leg\_left, }\AttributeTok{y =}\NormalTok{ height)) }\SpecialCharTok{+}
  ...
  \FunctionTok{annotate}\NormalTok{(}\StringTok{"text"}\NormalTok{,}
           \AttributeTok{x =} \FunctionTok{min}\NormalTok{(d}\SpecialCharTok{$}\NormalTok{leg\_left, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
           \AttributeTok{y =} \FunctionTok{max}\NormalTok{(d}\SpecialCharTok{$}\NormalTok{height, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
           \AttributeTok{label =} \FunctionTok{reg\_eq}\NormalTok{(mod2, }\StringTok{"Leg Left"}\NormalTok{, }\StringTok{"Height"}\NormalTok{),}
           \AttributeTok{hjust =} \DecValTok{0}\NormalTok{, }\AttributeTok{vjust =} \DecValTok{1}\NormalTok{, }\AttributeTok{size =} \FloatTok{3.5}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Leg Left"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Height"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"Leg Left vs Height"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme}\NormalTok{(}\AttributeTok{plot.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{))}

\CommentTok{\# Plot 3: Leg Right vs Height}
\NormalTok{p3 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(d, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ leg\_right, }\AttributeTok{y =}\NormalTok{ height)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{color =} \StringTok{"blue"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{, }\AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{color =} \StringTok{"black"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{annotate}\NormalTok{(}\StringTok{"text"}\NormalTok{,}
           \AttributeTok{x =} \FunctionTok{min}\NormalTok{(d}\SpecialCharTok{$}\NormalTok{leg\_right, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
           \AttributeTok{y =} \FunctionTok{max}\NormalTok{(d}\SpecialCharTok{$}\NormalTok{height, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
           \AttributeTok{label =} \FunctionTok{reg\_eq}\NormalTok{(mod3, }\StringTok{"Leg Right"}\NormalTok{, }\StringTok{"Height"}\NormalTok{),}
           \AttributeTok{hjust =} \DecValTok{0}\NormalTok{, }\AttributeTok{vjust =} \DecValTok{1}\NormalTok{, }\AttributeTok{size =} \FloatTok{3.5}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Leg Right"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Height"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"Leg Right vs Height"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme}\NormalTok{(}\AttributeTok{plot.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{))}

\CommentTok{\# Combine the three plots in a single row using patchwork}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-80-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Fit the model with both leg lengths}
...
\begin{verbatim}
##            mean        sd       5.5%    94.5%
## a     0.9811946 0.2839605  0.5273710 1.435018
## bl    0.2138690 2.5270778 -3.8248894 4.252627
## br    1.7816829 2.5312914 -2.2638096 5.827175
## sigma 0.6171136 0.0434362  0.5476942 0.686533
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-80-2.pdf}}

\textbf{With the given regression model, one asks the question}:
``What is the value of knowing each leg's length, after
already knowing the other leg's length, with respect to height?''

The answer is: ``Not much.'', since they are highly correlated, which - in this case - means
that if I know one I know the other.
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-81-1.pdf}}

Since the two coefficients are almost perfectly multicollinear
(\texttt{cor(d\$leg\_left,\ d\$leg\_right)=0.9997458}), already knowing the left leg's
...
In the example below, the second predictor \(X_2\) is a \emph{perfect} linear
function of the first predictor \(X_1\).
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-83-1.pdf}}
...
\item
  ...
  \end{itemize}
\item
  \textbf{Draw a DAG} (directed acyclic graph) of the hypothesized relationships between
...
design and missing values for now.
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-86-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Age \_||\_ BMI | Gender, PhyA}
...
which can be criticized. Let's quickly visualize the distribution of \emph{BPSysAve} and overlay
a normal density with parameters estimated from the data (\(\mu\) sample mean of \emph{BPSysAve},
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-89-1.pdf}}
...
Note that we implicitly assumed below that \(PhysActive=1\) (person is physically active);
...
This yields also negative values (please verify) for \emph{BPSysAve}, which is biologically not plausible.

In \hyperref[exercise15_multiple_regression]{exercise 15} we will play around with the priors
until the prior predictive check produces plausible values.
...
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{123}\NormalTok{)}
\NormalTok{n\_sims }\OtherTok{\textless{}{-}} \FunctionTok{dim}\NormalTok{(df\_age)[}\DecValTok{1}\NormalTok{] }\CommentTok{\# 7235 participants}
\NormalTok{beta\_0\_vec }\OtherTok{\textless{}{-}} \FunctionTok{rnorm}\NormalTok{(n\_sims, }\DecValTok{120}\NormalTok{, }\DecValTok{2}\NormalTok{)}
\NormalTok{beta\_1\_vec }\OtherTok{\textless{}{-}} \FunctionTok{rnorm}\NormalTok{(n\_sims, }\DecValTok{0}\NormalTok{, }\DecValTok{5}\NormalTok{)}
\NormalTok{beta\_2\_vec }\OtherTok{\textless{}{-}} \FunctionTok{rnorm}\NormalTok{(n\_sims, }\FloatTok{0.5}\NormalTok{, }\DecValTok{1}\NormalTok{)}
\NormalTok{beta\_3\_vec }\OtherTok{\textless{}{-}} \FunctionTok{rnorm}\NormalTok{(n\_sims, }\DecValTok{0}\NormalTok{, }\DecValTok{5}\NormalTok{)}
\NormalTok{sigma\_vec }\OtherTok{\textless{}{-}} \FunctionTok{runif}\NormalTok{(n\_sims, }\DecValTok{0}\NormalTok{, }\DecValTok{20}\NormalTok{)}
\NormalTok{age\_vec }\OtherTok{\textless{}{-}} \FunctionTok{sample}\NormalTok{(df\_age}\SpecialCharTok{$}\NormalTok{Age}\SpecialCharTok{{-}}\FunctionTok{mean}\NormalTok{(df\_age}\SpecialCharTok{$}\NormalTok{Age), n\_sims, }\AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{phys\_act }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(}\FunctionTok{sample}\NormalTok{(df\_age}\SpecialCharTok{$}\NormalTok{PhysActive, n\_sims, }\AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{))}\SpecialCharTok{{-}}\DecValTok{1}
\NormalTok{gender\_vec }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(}\FunctionTok{sample}\NormalTok{(df\_age}\SpecialCharTok{$}\NormalTok{Gender, n\_sims, }\AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{))}\SpecialCharTok{{-}}\DecValTok{1}
\NormalTok{BPSysAve\_sim }\OtherTok{\textless{}{-}} \FunctionTok{rnorm}\NormalTok{(n\_sims, beta\_0\_vec }\SpecialCharTok{+} 
\NormalTok{                        beta\_1\_vec}\SpecialCharTok{*}\NormalTok{phys\_act }\SpecialCharTok{+} 
\NormalTok{                        beta\_2\_vec}\SpecialCharTok{*}\NormalTok{age\_vec }\SpecialCharTok{+} 
\NormalTok{                        beta\_3\_vec}\SpecialCharTok{*}\NormalTok{gender\_vec, sigma\_vec)}
\FunctionTok{hist}\NormalTok{(BPSysAve\_sim, }\AttributeTok{breaks =} \DecValTok{50}\NormalTok{, }\AttributeTok{main =} \StringTok{"Prior predictive checks for BPSysAve"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-91-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-91-2.pdf}}
...
\begin{verbatim}
##                   mean         sd        5.5%       94.5%
## beta_0    120.43793815 5.73485228 111.2725366 129.6033397
## beta_1[1]   0.70843931 5.76687571  -8.5081419   9.9250205
## beta_1[2]  -0.02474661 5.76663823  -9.2409483   9.1914550
## beta_2      0.41947162 0.01115087   0.4016504   0.4372929
## beta_3[1]  -1.99172757 5.76727876 -11.2089529   7.2254978
## beta_3[2]   2.49240098 5.76733238  -6.7249101  11.7097120
## sigma      15.41591706 0.13105820  15.2064607  15.6253734
\end{verbatim}
...
\begin{verbatim}
##                         mean         sd        5.5%       94.5%
## beta_0          120.49716325 5.70549830 111.2581782 129.4574410
## beta_2            0.41950080 0.01110319   0.4015933   0.4373822
## sigma            15.41533138 0.13000680  15.2093178  15.6258113
## beta_1[1]         0.68257968 5.79584162  -8.7383707   9.8310725
## beta_1[2]        -0.05658728 5.79535440  -9.4738416   9.1218789
## beta_3[1]        -2.02522868 5.78318671 -11.2312134   7.1065450
## beta_3[2]         2.45988941 5.78179919  -6.7435892  11.5947876
## diff_PhysActive  -0.73916696 0.37861283  -1.3443667  -0.1358485
## diff_G            4.48511808 0.37136434   3.8851013   5.0768229
\end{verbatim}
...
My guess is: no. A first glance into the \href{https://journals.sagepub.com/doi/10.1177/00368504221101636}{literature}
mentions an MCID of 2 mmHG for resting systolic blood pressure.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{check\_model}\NormalTok{(mod, }\AttributeTok{check =} \StringTok{"linearity"}\NormalTok{) }\CommentTok{\# only linearity}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-93-1.pdf}}
...
There also seems to be some heteroscedasticity and the residuals are not normally distributed.

\textbf{By the way, usually}, one would fit this model by throwing in all variables into the model, \textbf{including BMI}.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{check\_model}\NormalTok{(mod) }\CommentTok{\# show all plots}
...
Since \(BMI\) is on a pipe from \emph{PhysActive} to \emph{BPSysAve}, it makes sense,
that the effect of \emph{PhysActive} is smaller when we include \(BMI\) in the model.
We ask: ``What is the (additional) effect of physical activity on blood pressure,
given \(BMI\)?''.
...
(Note: It is surprisingly hard to include a left-skewed distribution in \texttt{rethinking}.
Currently trying to resolve this issue.)

\textbf{Second}, we further try to model \(\sigma\) as a function of \(\mu\).
For higher expected values of \emph{BPSysAve}, we would expect higher variability.
Hence, we try to \textbf{explicitly model heteroscedasticity}.
One could argue that the exponential function is not the best choice due
to numerical problems.

Note, that we have no problem with adding parameters to the model
because of the large sample size.

The prior for \(\beta_0\) is now \(5\) (not \(140\)) since we have a
log-normal distribution. In the R code, I have calibrated this graphically.
Also: \(log(140) \approx 5\)
...
\CommentTok{\# calibrate parameters for priors.}
\FunctionTok{hist}\NormalTok{(}\FunctionTok{rlnorm}\NormalTok{(}\DecValTok{10000}\NormalTok{, }\AttributeTok{meanlog =} \DecValTok{5}\NormalTok{, }\AttributeTok{sdlog =} \FloatTok{0.5}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-95-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# {-}\textgreater{} meanlog should be around 5!, not 140!}
...
\NormalTok{    beta\_0 }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\DecValTok{5}\NormalTok{, }\DecValTok{10}\NormalTok{),}
\NormalTok{    beta\_1[PhysActive] }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{),}
\NormalTok{    beta\_2 }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{),}
\NormalTok{    beta\_3[Gender] }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{),}
\NormalTok{    beta\_4 }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{),}
\NormalTok{    beta\_5 }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{)}
\NormalTok{  ),}
  \AttributeTok{data =}\NormalTok{ df,}
  \AttributeTok{start =} \FunctionTok{list}\NormalTok{(}\AttributeTok{beta\_0 =} \DecValTok{4}\NormalTok{, }\AttributeTok{beta\_1 =} \FunctionTok{c}\NormalTok{(}\FloatTok{0.5}\NormalTok{, }\DecValTok{0}\NormalTok{), }
  \AttributeTok{beta\_2 =} \DecValTok{0}\NormalTok{, }\AttributeTok{beta\_3 =} \FunctionTok{c}\NormalTok{(}\FloatTok{0.5}\NormalTok{, }\DecValTok{0}\NormalTok{), }\AttributeTok{beta\_4 =} \DecValTok{1}\NormalTok{, }\AttributeTok{beta\_5 =} \DecValTok{0}\NormalTok{)}
\NormalTok{)}
...
\begin{verbatim}
##                  mean           sd          5.5%         94.5%
## beta_0      4.1293689 7.069699e+00  -7.169375695  15.428113525
## beta_1[1]  -0.7777471 6.122238e+00 -10.562265122   9.006770975
## beta_1[2]  -0.7809708 6.122238e+00 -10.565488880   9.003547193
## beta_2      0.0033247 8.493131e-05   0.003188964   0.003460437
## beta_3[1]   1.4153224 6.123815e+00  -8.371716586  11.202361359
## beta_3[2]   1.4569511 6.123815e+00  -8.330087946  11.243990082
## beta_4    -14.7636798 7.497431e-01 -15.961914016 -13.565445546
## beta_5      2.6408675 1.567597e-01   2.390335221   2.891399779
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Expected difference in blood pressure (mmHg):}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{122}\NormalTok{)}
\NormalTok{samples }\OtherTok{\textless{}{-}} \FunctionTok{extract.samples}\NormalTok{(m\_NHANES\_lnorm)}

\NormalTok{mu\_active  }\OtherTok{\textless{}{-}}\NormalTok{ samples}\SpecialCharTok{$}\NormalTok{beta\_0 }\SpecialCharTok{+}\NormalTok{ samples}\SpecialCharTok{$}\NormalTok{beta\_1[, }\DecValTok{2}\NormalTok{]}
\NormalTok{mu\_inactive }\OtherTok{\textless{}{-}}\NormalTok{ samples}\SpecialCharTok{$}\NormalTok{beta\_0 }\SpecialCharTok{+}\NormalTok{ samples}\SpecialCharTok{$}\NormalTok{beta\_1[, }\DecValTok{1}\NormalTok{]}

\CommentTok{\# otherwise we get numerical explosions...}
\NormalTok{clip }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(x, }\AttributeTok{lower =} \SpecialCharTok{{-}}\DecValTok{100}\NormalTok{, }\AttributeTok{upper =} \DecValTok{100}\NormalTok{) }\FunctionTok{pmin}\NormalTok{(}\FunctionTok{pmax}\NormalTok{(x, lower), upper)}

\NormalTok{sigma\_active }\OtherTok{\textless{}{-}} \FunctionTok{exp}\NormalTok{(}\FunctionTok{clip}\NormalTok{(samples}\SpecialCharTok{$}\NormalTok{beta\_4 }\SpecialCharTok{+}\NormalTok{ samples}\SpecialCharTok{$}\NormalTok{beta\_5 }\SpecialCharTok{*}\NormalTok{ mu\_active))}
\NormalTok{sigma\_inactive }\OtherTok{\textless{}{-}} \FunctionTok{exp}\NormalTok{(}\FunctionTok{clip}\NormalTok{(samples}\SpecialCharTok{$}\NormalTok{beta\_4 }\SpecialCharTok{+}\NormalTok{ samples}\SpecialCharTok{$}\NormalTok{beta\_5 }\SpecialCharTok{*}\NormalTok{ mu\_inactive))}

\CommentTok{\# Expected values on BPSysAve scale}
\NormalTok{E\_active }\OtherTok{\textless{}{-}} \FunctionTok{exp}\NormalTok{(}\FunctionTok{clip}\NormalTok{(mu\_active }\SpecialCharTok{+} \FloatTok{0.5} \SpecialCharTok{*}\NormalTok{ sigma\_active}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{))}
\NormalTok{E\_inactive }\OtherTok{\textless{}{-}} \FunctionTok{exp}\NormalTok{(}\FunctionTok{clip}\NormalTok{(mu\_inactive }\SpecialCharTok{+} \FloatTok{0.5} \SpecialCharTok{*}\NormalTok{ sigma\_inactive}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{))}

\CommentTok{\# (distribution of) Expected difference in blood pressure (mmHg)}
\NormalTok{diff\_expected\_BP }\OtherTok{\textless{}{-}}\NormalTok{ E\_active }\SpecialCharTok{{-}}\NormalTok{ E\_inactive}

\CommentTok{\# Clean}
\NormalTok{diff\_expected\_BP\_clean }\OtherTok{\textless{}{-}}\NormalTok{ diff\_expected\_BP[}\FunctionTok{is.finite}\NormalTok{(diff\_expected\_BP) }\SpecialCharTok{\&} 
                                          \FunctionTok{abs}\NormalTok{(diff\_expected\_BP) }\SpecialCharTok{\textless{}} \DecValTok{20}\NormalTok{]}

\FunctionTok{precis}\NormalTok{(}\FunctionTok{data.frame}\NormalTok{(diff\_expected\_BP\_clean))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##                              mean       sd       5.5%        94.5% histogram
## diff_expected_BP_clean -0.1723838 1.239007 -0.4618121 0.0001112856  ▁▁▁▇▁▁▁▁
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{length}\NormalTok{(diff\_expected\_BP\_clean) }
\end{Highlighting}
\end{Shaded}
...
To get the expected difference in blood pressure (in mmHg) between
active and inactive people, we first need to calculate the difference in the
\emph{parameters} of the log-normal distribution.
After that, we use the parameters to calculate the difference
in expected values of the log-normal distribution.
\[ \mu_{LogNormal} = \exp(\mu + \frac{\sigma^2}{2})\]
This way, we get the desired result.
\(9485\) samples from \(10000\) remain after cutting off the extreme values.
The mean expected difference in blood pressure is \(-0.17\) mmHg with a 89\%
credible interval of approximately \((-0.5, 0)\) mmHg, which is not
very different from our first model.

\textbf{Next}, let's visualize the posterior predictive distributions of 100 samples:
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-96-1.pdf}}
...
from the first model disappeared.
...
\item
  How big is \(R^2\)?
\item
  Could you have calculated this without \texttt{lm}and R?
\end{itemize}
...
\item
  Plot a scatterplot of the math score and the verbal score and color the points according
...
Go back to the interpretation of the intercept in the quadratic height model \hyperref[interpretation_output_freq_quadratic]{above}.
...
\subsection{{[}M{]} Exercise 19}\label{exercise19_multiple_regression}

Consider the example from the book on page 189 McElreath: 6M3.

\begin{itemize}
\tightlist
\item
  For each of the four DAGs below, state which variables, if any, you must
  adjust for (condition on) to estimate the total causal effect of \(X\) on \(Y\).
\item
  Section 6.4 in the book explains how to do this.
\item
  Verify your answer with dagitty.net
\end{itemize}

\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{bookdown-demo_files/images/DAGS_6M3_McEleath.png}

\subsection{{[}M{]} Exercise 20}\label{exercise20_multiple_regression}

Consider the DAG in McElreath page 187 where one wants to determine
the total causal effect of \emph{Number of waffle houses} (W) on
\emph{divorce rate} (D).
...
\item
  Verify that both adjustment sets \(\{A, M\} \text{ and } \{S\}\)
...
\begin{itemize}
\item
  Repeat the \textbf{entire content} we have covered in this lecture so far.
  Try to grasp the big-picture view of what we want to achieve.
  Specifically, \textbf{note all your open questions} about our content so far
  and send them to me via email. I will try to answer every one of them
  in our exercise session.
\item
  Read chapters in the books from Westfall and McElreath to deepen your understanding, specifically the ones
  we have referred to in the script.
\item
  Go through the \href{https://jdegenfellner.github.io/Script_QM2_ZHAW/multiple-linear-regression.html\#example_nhanes}{NHANES example}
  till the end and try to understand the steps taken.
\item
  Go through the slides of the smaller version of the NHANES example (in Moodle, Week 3) as well.
  We will - of course - discuss the NHANES example in detail in class.
\item
  ...
  \tightlist
  \item
    Try to criticize the paper with respect to the content we have covered so far.
  \item
  ...
  \item
    What would you do differently if you were to perform that study?
  \item
    Are the conclusions of the paper justified by the data and the analysis?
  \end{itemize}
\end{itemize}
...
\item
  Try to find a paper in the field of physiotherapy which uses a fully Bayesian approach without any
  \(p\)-values and critique it with respect to the content we have covered so far.
  If you can't find one in the field of physiotherapy, you can also look for one in a related field.
  Don't give up easily, these papers might be hard to find - even in high-impact journals.

  \begin{itemize}
  \tightlist
  \item
    How did they justify their choice of priors?
  \item
    What model checks were performed?
  \item
    Are the conclusions justified by the data and the analysis?
  \end{itemize}
\item
  Simulate fictitious (but not too exotic) relationships between the variables in the NHANES example (as was done
  for the simple cases of the pipe, fork and collider)
  and verify that you get the total causal effect of physical activity on blood pressure
  when you do not include BMI in the model,
  but only the direct effect when you include BMI in the model.
\item
  Glimpse into our next chapter (if there is time left):
  \href{https://jdegenfellner.github.io/Script_QM3_ZHAW/reliability-and-validity.html}{Reliability}
\end{itemize}
...
Welche(s) Regressionsmodell(e) ist/sind \emph{linear}?
...
\tightlist
\item
  Interaktionsterme können nur zwischen kategorialen Variablen verwendet werden.
\item
  Der Effekt von \(X_1\) auf \(Y\) hängt davon ab, welchen Wert \(X_2\) hat.
\item
  Das Modell ist nicht mehr linear, weil der Interaktionsterm \(X_1 \cdot X_2\) enthalten ist.
\item
  Der Effekt von \(X_2\) auf \(Y\) hängt davon ab, welchen Wert \(X_1\) hat.
\end{itemize}
...
\NormalTok{sim\_cor }\OtherTok{\textless{}{-}} \FunctionTok{numeric}\NormalTok{(}\DecValTok{1000}\NormalTok{)}
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\DecValTok{1000}\NormalTok{) \{}
  \CommentTok{\# Simulate data}
  \CommentTok{\# X: Income (in 1000s), right{-}skewed}
\NormalTok{  X }\OtherTok{\textless{}{-}} \FunctionTok{rgamma}\NormalTok{(N, }\AttributeTok{shape =} \DecValTok{2}\NormalTok{, }\AttributeTok{rate =} \FloatTok{0.1}\NormalTok{)}
  
  \CommentTok{\# Z: Stress events per day \textasciitilde{} Poisson, depends on income}
\NormalTok{  lambda\_Z }\OtherTok{\textless{}{-}} \FunctionTok{exp}\NormalTok{(}\DecValTok{3} \SpecialCharTok{{-}} \FloatTok{0.05} \SpecialCharTok{*}\NormalTok{ X)}
\NormalTok{  Z }\OtherTok{\textless{}{-}} \FunctionTok{rpois}\NormalTok{(N, }\AttributeTok{lambda =}\NormalTok{ lambda\_Z)}
  
  \CommentTok{\# Y: Sleep quality (1 = poor sleep), depends on stress level}
\NormalTok{  p\_Y }\OtherTok{\textless{}{-}} \FunctionTok{plogis}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{2} \SpecialCharTok{+} \FloatTok{0.5} \SpecialCharTok{*}\NormalTok{ Z)}
\NormalTok{  Y }\OtherTok{\textless{}{-}} \FunctionTok{rbinom}\NormalTok{(N, }\AttributeTok{size =} \DecValTok{1}\NormalTok{, }\AttributeTok{prob =}\NormalTok{ p\_Y)}
  
  \CommentTok{\# Calculate cond correlation between X and Y}
\NormalTok{  sim\_cor[i] }\OtherTok{\textless{}{-}} \FunctionTok{cor}\NormalTok{(X[Z }\SpecialCharTok{==} \DecValTok{3}\NormalTok{], Y[Z }\SpecialCharTok{==} \DecValTok{3}\NormalTok{])  }\CommentTok{\# Correlation when Z = 3}
\NormalTok{\}}

\FunctionTok{mean}\NormalTok{(sim\_cor)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Wenn man den Wert von Z kennt, trägt X nichts zur Erklärung von Y bei.
\item
  Der Code simuliert einen sog. \emph{Pipe}.
\item
  Der zugehörige DAG ist \(X \rightarrow Z \rightarrow Y\).
\item
  \(cor(X, Y) \ne 0\).
\end{itemize}
