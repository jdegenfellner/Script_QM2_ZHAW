...
\begin{verbatim}
##             mean        sd       5.5%      94.5%
## mu    154.603927 0.4119990 153.945473 155.262381
## sigma   7.731421 0.2913942   7.265716   8.197125
\end{verbatim}
...
\begin{verbatim}
##                 mu        sigma
## mu    0.1697431600 0.0001500467
## sigma 0.0001500467 0.0849105951
\end{verbatim}
...
\begin{verbatim}
##                mu       sigma
## mu    1.000000000 0.001249825
## sigma 0.001249825 1.000000000
\end{verbatim}
...
\begin{verbatim}
## file:////private/var/folders/pm/jd6n6gj10371_bml1gh8sc5w0000gn/T/Rtmp1cF8Ch/file108aa1ce42153/widget108aa4547def6.html screenshot completed
\end{verbatim}
...
\begin{verbatim}
## file:////private/var/folders/pm/jd6n6gj10371_bml1gh8sc5w0000gn/T/Rtmp1cF8Ch/file108aa750d1eb3/widget108aa305ab5ce.html screenshot completed
\end{verbatim}
...
\begin{verbatim}
## file:////private/var/folders/pm/jd6n6gj10371_bml1gh8sc5w0000gn/T/Rtmp1cF8Ch/file108aa5eb7c207/widget108aa72238a25.html screenshot completed
\end{verbatim}
...
\begin{verbatim}
## file:////private/var/folders/pm/jd6n6gj10371_bml1gh8sc5w0000gn/T/Rtmp1cF8Ch/file108aa4db37b05/widget108aa4251ad1d.html screenshot completed
\end{verbatim}
...
\begin{verbatim}
## file:////private/var/folders/pm/jd6n6gj10371_bml1gh8sc5w0000gn/T/Rtmp1cF8Ch/file108aa4c84e21e/widget108aa281946d1.html screenshot completed
\end{verbatim}
...
Does \emph{Physical activity} (PhysActive) influence the \emph{average systolic
blood pressure} (BPSysAve) in adults (\(\ge 20\) years)?

We propose the following relationships among the variables
(we limit ourselves to 4 covariates):
...
age, BMI, gender, and blood pressure (BPSysAve). Physical activity directly
...
be less active and may experience weight gain due to metabolic changes.
...
and physical activity levels due to both biological and societal factors.

Note, that we have a very large sample size (10,000 rows). As exercise for later,
...
The one adjustment set proposed by \texttt{dagitty} tells us that we should include
Age and Gender as covariates in the model, if the DAG is correct.
Note that we do not include all three covariables in the model, just two of them.

We should probably check the implied conditional independencies to see how this is
done:

First, we need to test, if Age and BMI are independent, \emph{if} we condition on Gender and Physical Activity.
This is the case.
...
This implication can be confirmed.
Interpretation: If we know the Gender and Physical Activity of a person,
...
between Age and Gender, at least not statistically relevant.
...
We could try a \textbf{multiple linear regression model} here.

As a first approximation, let's assume BPSysAve is (sufficiently) normally distributed,
which can be critized. Let's quickly visualize the distribution of BPSysAve:
...
\NormalTok{df\_age }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \AttributeTok{bins =} \DecValTok{30}\NormalTok{, }\AttributeTok{fill =} \StringTok{"lightblue"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.6}\NormalTok{) }\SpecialCharTok{+}  \CommentTok{\# Ensure density scaling}
  \FunctionTok{geom\_density}\NormalTok{(}\AttributeTok{color =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{linewidth =} \DecValTok{1}\NormalTok{) }\SpecialCharTok{+}  \CommentTok{\# Smoothed density of actual data}
...
We choose vague priors for now since we have a vast data set.
...
\item
  ...
  holding all other predictors constant. BPSysAve ranges from 78 to 226.
...
\item
  ...
  since we are dealing with a one-year increase in Age.
\item
  \(\sigma\): The standard deviation of the residuals.
  \(\sigma \sim \text{Uniform}(0, 50)\). We use a vague prior.
  95\% of the residuals are between -100 and 100. This seems plausible
\end{itemize}
...
This yields also negative values for BPSysAve, which is not plausible.
...
\NormalTok{  ) }\SpecialCharTok{+}
  \FunctionTok{scale\_fill\_manual}\NormalTok{(}\AttributeTok{values =} \FunctionTok{c}\NormalTok{(}\StringTok{"Observed"} \OtherTok{=} \StringTok{"blue"}\NormalTok{, }\StringTok{"Simulated"} \OtherTok{=} \StringTok{"red"}\NormalTok{)) }\SpecialCharTok{+}  \CommentTok{\# Custom colors}
  \FunctionTok{theme\_minimal}\NormalTok{() }\SpecialCharTok{+} 
  \FunctionTok{theme}\NormalTok{(}\AttributeTok{plot.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{df }\OtherTok{\textless{}{-}}\NormalTok{ df }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}
    \AttributeTok{PhysActive\_index =} \FunctionTok{as.integer}\NormalTok{(}\FunctionTok{as.factor}\NormalTok{(PhysActive)),  }\CommentTok{\# Convert to factor index (1{-}based)}
    \AttributeTok{Gender\_index =} \FunctionTok{as.integer}\NormalTok{(}\FunctionTok{as.factor}\NormalTok{(Gender))  }\CommentTok{\# Convert to factor index (1{-}based)}
\NormalTok{  )}
...
\NormalTok{Age\_mean }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{(df[Age }\SpecialCharTok{\textgreater{}=} \DecValTok{20}\NormalTok{,]}\SpecialCharTok{$}\NormalTok{Age, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
...
\subsubsection{\texorpdfstring{Fit model using \texttt{lm}}{Fit model using lm}}\label{fit-model-using-lm}
...
Usually, one would fit the model by throwing in all variables including BMI.
Let's see if this approach changes the results:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mod }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(BPSysAve }\SpecialCharTok{\textasciitilde{}}\NormalTok{ PhysActive }\SpecialCharTok{+}\NormalTok{ Age }\SpecialCharTok{+}\NormalTok{ Gender  }\SpecialCharTok{+}\NormalTok{ BMI, }\AttributeTok{data =}\NormalTok{ df)}
\FunctionTok{summary}\NormalTok{(mod)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{qqPlot}\NormalTok{(mod) }\CommentTok{\# }
\end{Highlighting}
\end{Shaded}
...
This changed the results notable. Before, the effect of physical activity was \$ -0.73693\$
with a rather small \(p\)-value,
not it is \(-0.22870\) with a \(p\)-value ten times as large.

\textbf{Notice this}: The interpretation of a whole paper could change by falsely including
and additional variable (in this case BMI).

\subsubsection{Improve model}\label{improve_model_NHANES}

Now, that we have seen that the model fit is not perfect, we could try to improve it.
...
Let's try a gamma distribution for the outcome variable:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(fitdistrplus)  }\CommentTok{\# For fitting distributions}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Remove NA values before fitting}
\NormalTok{bps\_values }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(}\FunctionTok{na.omit}\NormalTok{(df\_age}\SpecialCharTok{$}\NormalTok{BPSysAve))}

\CommentTok{\# Fit a Gamma distribution}
\NormalTok{gamma\_fit }\OtherTok{\textless{}{-}} \FunctionTok{fitdist}\NormalTok{(bps\_values, }\StringTok{"gamma"}\NormalTok{)}
\NormalTok{gamma\_fit}\SpecialCharTok{$}\NormalTok{estimate}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{ggplot}\NormalTok{(df\_age, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ BPSysAve)) }\SpecialCharTok{+}
  \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ ..density..), }\AttributeTok{bins =} \DecValTok{30}\NormalTok{, }\AttributeTok{fill =} \StringTok{"lightblue"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.6}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_density}\NormalTok{(}\AttributeTok{color =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{size =} \DecValTok{1}\NormalTok{) }\SpecialCharTok{+}  \CommentTok{\# Smoothed density of actual data}
  \FunctionTok{stat\_function}\NormalTok{(}
    \AttributeTok{fun =}\NormalTok{ dgamma,}
    \AttributeTok{args =} \FunctionTok{list}\NormalTok{(}\AttributeTok{shape =}\NormalTok{ gamma\_fit}\SpecialCharTok{$}\NormalTok{estimate[}\StringTok{"shape"}\NormalTok{], }\AttributeTok{rate =}\NormalTok{ gamma\_fit}\SpecialCharTok{$}\NormalTok{estimate[}\StringTok{"rate"}\NormalTok{]),}
    \AttributeTok{color =} \StringTok{"purple"}\NormalTok{, }\AttributeTok{size =} \DecValTok{1}\NormalTok{, }\AttributeTok{linetype =} \StringTok{"dashed"}
\NormalTok{  ) }\SpecialCharTok{+}  \CommentTok{\# Gamma distribution}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{x =} \StringTok{"Systolic Blood Pressure (BPSysAve)"}\NormalTok{, }
    \AttributeTok{y =} \StringTok{"Density"}\NormalTok{, }
    \AttributeTok{title =} \StringTok{"Distribution of Systolic Blood Pressure with Weibull \& Gamma Fits"}
\NormalTok{  ) }\SpecialCharTok{+}
  \FunctionTok{theme\_minimal}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{theme}\NormalTok{(}\AttributeTok{plot.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{))}
\end{Highlighting}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/figure-latex/unnamed-chunk-94-1.pdf}}

Looks better than the normal distribution. We could have also used vague
priors for the gamma parameters since we have a large sample size.
...
Note, that we have not problem with adding parameters to the model
because of the large sample size.

See \hyperref[exercise16_multiple_regression]{exercise 16}.

\begin{eqnarray*}
BPSysAve_i &\sim& \text{Normal}(\mu_i, \sigma_i)\\
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m\_NHANES\_gamma }\OtherTok{\textless{}{-}} \FunctionTok{quap}\NormalTok{(}
  \FunctionTok{alist}\NormalTok{(}
\NormalTok{    BPSysAve }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(mu, sigma), }
\NormalTok{    sigma }\OtherTok{\textless{}{-}} \FunctionTok{exp}\NormalTok{(beta\_4 }\SpecialCharTok{+}\NormalTok{ beta\_5 }\SpecialCharTok{*}\NormalTok{ mu), }\CommentTok{\# new}
\NormalTok{    mu }\OtherTok{\textless{}{-}}\NormalTok{ beta\_0 }\SpecialCharTok{+}\NormalTok{ beta\_1[PhysActive] }\SpecialCharTok{+}\NormalTok{ beta\_2 }\SpecialCharTok{*}\NormalTok{ (Age }\SpecialCharTok{{-}}\NormalTok{ Age\_mean) }\SpecialCharTok{+}\NormalTok{ beta\_3[Gender],}
\NormalTok{    beta\_0 }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\DecValTok{140}\NormalTok{, }\DecValTok{10}\NormalTok{), }
\NormalTok{    beta\_1[PhysActive] }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{), }
\NormalTok{    beta\_2 }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{),}
\NormalTok{    beta\_3[Gender] }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{),  }
\NormalTok{    beta\_4 }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{),  }\CommentTok{\# new}
\NormalTok{    beta\_5 }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{)  }\CommentTok{\# new}
\NormalTok{  ),}
  \AttributeTok{data =}\NormalTok{ df,}
...
\FunctionTok{precis}\NormalTok{(m\_NHANES\_gamma, }\AttributeTok{depth =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##                  mean          sd         5.5%        94.5%
## beta_0    129.9078481 7.071283091 118.60657197 141.20912422
## beta_1[1]   1.3822164 6.125779091  -8.40796173  11.17239451
## beta_1[2]   1.1772245 6.125726274  -8.61286917  10.96731825
## beta_2      0.4092944 0.011085607   0.39157749   0.42701137
## beta_3[1] -12.1511824 6.125369428 -21.94070579  -2.36165900
## beta_3[2]  -7.2480282 6.125912352 -17.03841929   2.54236291
## beta_4     -1.4582152 0.177759518  -1.74230925  -1.17412117
## beta_5      0.0341528 0.001465127   0.03181124   0.03649436
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# dfference in levels of PhysActive and Gender:}
\NormalTok{post\_gamma }\OtherTok{\textless{}{-}} \FunctionTok{extract.samples}\NormalTok{(m\_NHANES\_gamma)}
\NormalTok{post\_gamma}\SpecialCharTok{$}\NormalTok{diff\_PhysActive }\OtherTok{\textless{}{-}}\NormalTok{ post\_gamma}\SpecialCharTok{$}\NormalTok{beta\_1[,}\DecValTok{2}\NormalTok{] }\SpecialCharTok{{-}}\NormalTok{ post\_gamma}\SpecialCharTok{$}\NormalTok{beta\_1[,}\DecValTok{1}\NormalTok{]}
\NormalTok{post\_gamma}\SpecialCharTok{$}\NormalTok{diff\_G }\OtherTok{\textless{}{-}}\NormalTok{ post\_gamma}\SpecialCharTok{$}\NormalTok{beta\_3[,}\DecValTok{2}\NormalTok{] }\SpecialCharTok{{-}}\NormalTok{ post\_gamma}\SpecialCharTok{$}\NormalTok{beta\_3[,}\DecValTok{1}\NormalTok{]}
\FunctionTok{precis}\NormalTok{(post\_gamma, }\AttributeTok{depth =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##                         mean          sd         5.5%        94.5%
## beta_0          129.86586686 7.137410288 118.58553593 141.19831108
## beta_2            0.40907839 0.011269698   0.39119695   0.42702715
## beta_4           -1.46141062 0.177139505  -1.74802663  -1.18097688
## beta_5            0.03417879 0.001460823   0.03186429   0.03654536
## beta_1[1]         1.39572198 6.122237159  -8.39747051  11.29040125
## beta_1[2]         1.19094674 6.121597482  -8.58308798  11.09139910
## beta_3[1]       -12.12521938 6.162147944 -21.89613320  -2.22341805
## beta_3[2]        -7.22448013 6.163484254 -17.02747162   2.69505942
## diff_PhysActive  -0.20477524 0.295404998  -0.67810759   0.26671736
## diff_G            4.90073925 0.307036706   4.41178074   5.38993341
##                       histogram
## beta_0            ▁▁▁▁▂▅▇▇▅▂▁▁▁
## beta_2               ▁▁▁▃▇▇▃▁▁▁
## beta_4          ▁▁▁▁▂▃▇▇▇▃▂▁▁▁▁
...
Let's vizualize the postior predictive distributions of 100 samples:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# histrogram of posterior of BPSysAve:}
\NormalTok{sample\_BP }\OtherTok{\textless{}{-}} \FunctionTok{sim}\NormalTok{(m\_NHANES\_gamma, }\AttributeTok{n =} \DecValTok{1000}\NormalTok{)}
...
\FunctionTok{ggplot}\NormalTok{() }\SpecialCharTok{+}
  \CommentTok{\# Original BPSysAve density in green}
  ...
  \CommentTok{\# Posterior densities as individual lines in blue with transparency}
...
See \hyperref[exercise16_multiple_regressionux5cux257D]{exercise 17}.

\subsubsection{Interpret and report results}\label{interpret-and-report-results}

Summarizing the results, we find the expected direction of association in our first and second model.
The effect of physical activity on blood pressure is negative. This would be probably more pronounced
if we had more than a binary variable for physical activity. Note that the effect
is rather tiny. Expected difference in blood pressure between physically active and non-active
people is around 0.2 mmHg with a large credible interval that includes 0. Without looking up,
one could assume, that the effect is not practically relevant.
